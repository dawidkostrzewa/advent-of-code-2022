...
fs.readFile('input-11.txt', 'utf8', (err, data) => {
    ...
    while(ROUNDS--) {
            for (let i = 0; i < monkeys.length; i++) {
                ...
                monkey.inspectedTimes += items.length
                items.forEach(it => {
                    const afterOperation = getResultOfOperation(operation, it);
                    const afterBored = afterOperation % divider;
    ...
    console.log("PART 2 ANSWER", twoMostInspected[0].inspectedTimes * twoMostInspected[1].inspectedTimes)
});
